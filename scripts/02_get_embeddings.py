...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Output prefix for .npy and .json files",
    )
    parser.add_argument(
        "--skip-merge", action="store_true", help="Skip merging files at the end"
    )
...
async def recursive_fetch_safe(
    texts: List[str], ids: List[str], semaphore: asyncio.Semaphore, depth: int = 0
    ...
    try:
        ...
        if depth > 0:
            print(
                f"  > Split depth {depth}: {len(texts)} -> {len(l_texts)} + {len(r_texts)}",
                file=sys.stderr,
            )
...
def merge_results(output_prefix: str):
    ...
    if sorted_indices[-1] != len(sorted_indices) - 1:
        print(
            f"Warning: Found {len(sorted_indices)} batches but max index is {sorted_indices[-1]}. There may be gaps."
        )
    ...
    merged_arr = open_memmap(
        final_npy_path, mode="w+", dtype=dtype, shape=(total_rows, embedding_dim)
    )
